...
writer.run(() => {
    // 私有：使用固定变量包装打印日志
    getContent = writer.bind(function getContent(...args) {
        return Format(...args);
    });

    format = writer.bind(function format(outputOpt, message) {
    ...
    getTraceId = writer.bind(function getTraceId() {
        let traceId;
    ...
    // 重新生成traceId
    genTraceId = writer.bind(function genTraceId() {
