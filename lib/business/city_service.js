...
class CityServiceLogger {
    ...
    getCityServiceLogger(loggerName) {
...
let middleware = function middleware(loggerName, options = {}) {
    ...
    let buildKoa2LoggerHandler = async function buildKoa2LoggerHandler(ctx, next) {
        ...
        if( !cityServiceLoggerMap[loggerName] ) {
            // nodeLoggerInstance = new BaseLogger(curOptions);
            nodeLoggerInstance = new BaseLogger(loggerName, curOptions);
            cityServiceLoggerMap[loggerName] = nodeLoggerInstance;
        }
