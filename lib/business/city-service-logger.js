...
class CityServiceLogger {
    ...
    static middleware(loggerName, options = {}) {
        ...
        let buildKoa2LoggerHandler = async function buildKoa2LoggerHandler(ctx, next) {
            ...
            let baseNodeLoggerInstance = BaseLogger.getOneBaseLogger(loggerName, curOptions);
    ...
    setBusiness(business) {
        this.business = business || PlaceHolder;
    ...
    debug(...args) {
        if (this.logger) {
            this.logger.debug(this.business, ...args);
    ...
    info(...args) {
        if (this.logger) {
            this.logger.info(this.business, ...args);
            this.refreshSecondaryProperty();
        } else {
            console.error('logger of LockCommonLogger is null');
        }
    ...
    warn(...args) {
        if (this.logger) {
            this.logger.warn(this.business, ...args);
            this.refreshSecondaryProperty();
        } else {
            console.error('logger of LockCommonLogger is null');
        }
    ...
    error(...args) {
        if (this.logger) {
            this.logger.error(this.business, ...args);
            this.refreshSecondaryProperty();
        } else {
            console.error('logger of LockCommonLogger is null');
        }
